...
export type BlindspotInsight = {
  ...
  userFollowCenter: number;
  storyCoverageCenter: number;
};
...
export function blindspotFromSpread(
  ...
  const userCenter =
    followLabels.reduce((s, l) => s + SCORE[l], 0) / followLabels.length;
  ...
  if (Math.abs(delta) < 0.6) {
    return {
      ...
      userFollowCenter: userCenter,
      storyCoverageCenter: storyCenter,
    };
  }
